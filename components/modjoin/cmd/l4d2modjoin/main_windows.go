...
const (
	...
	wmAppEvent       = 0x8001
	wmMixCanClose    = 0x80F0
	wmMixActivate    = 0x80F1
...
func main() {
	...
	parent := uintptr(0)
	x, y, width, height := 180, 90, 980, 720
	if embedParent != 0 {
	...
	hwnd, _, _ := procCreateWindow.Call(
		...
		windowStyle, uintptr(x), uintptr(y), uintptr(width), uintptr(height), parent, 0, instance, 0,
	)
	if hwnd == 0 {
		return
	}
...
func windowProc(hwnd uintptr, message uint32, wParam, lParam uintptr) uintptr {
	...
	case wmAppEvent:
		...
		return 0
	case wmMixCanClose:
		if ui.busy || resolver != nil {
			return 0
		}
		return 1
	case wmMixActivate:
		ensureConflictResolverOnTop()
		return 1
...
func createUI(hwnd uintptr) {
	...
	base := cwd
	stateRoot := cwd
	if configured := strings.TrimSpace(os.Getenv("L4D2_MIX_MOD_JOIN_ROOT")); configured != "" {
		stateRoot = filepath.Clean(configured)
		base = stateRoot
	...
	if configured := strings.TrimSpace(os.Getenv("L4D2_MIX_MOD_JOIN_STATE_DIR")); configured != "" {
		ui.stateDir = filepath.Clean(configured)
	} else {
